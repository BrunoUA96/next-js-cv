...
import { onChangeSection } from '@/lib/features/banner/bannerSlice';
...
export const ContentBox = ({ color }: { color: string }) => {
  return (
    <div
...
export const Box = ({
  ...
  const [countSection] = useDebounce(count, 600);
  const [viewSection] = useDebounce(inView, 600);

  useEffect(() => {
    if (countSection && inView) {
      dispatch(onChangeSection(countSection));
    }
  }, [viewSection, countSection]);

  return (
    <div
      className={`snap-child-center flex items-center`}
      ref={ref}
      style={{
        marginTop: 2,
        marginBottom: 2,
        height: full ? 'calc(100vh - 123px)' : half ? '50vh' : 120,
        backgroundColor: `rgba(${color} / 50%)`,
        justifyContent: contentCenter ? 'center' : 'end',
        width: '100%',
      }}
    >
      <motion.div
